<template>
  <section class="grid grid-cols-4 lg:grid-cols-5 h-full gap-9 overflow-auto lg:overflow-hidden p-2">
    <div class="col-span-4 lg:col-span-3">
      <div class="flex flex-col gap-4">
        ...
        <AppSelectInput v-model="state" name="condition" label="Оберіть стан" dataKey="id" :options="states" />
      </div>
      <div class="flex justify-center">
        <button class="p-3 flex flex-row" @click="toggle_more_info">
          <DoubleArrowDown /> <span>Додати інформацію</span>
          <DoubleArrowDown />
        </button>
      </div>
      <div v-if="add_more_info" class="grid grid-rows-7 sm:grid-flow-col gap-y-6 gap-x-6">
        ...
        <AppPlainInput name="surname" label="Прізвище" type="text" />
        ...
        <AppSelectInput v-model="gender" name="gender" label="Стать" dataKey="id" :options="genders" />
        <AppPlainInput name="condition" label="Стан" />
        <AppPlainInput name="specific_signs" label="Особливі прикмети" type="text" />
        <AppPlainInput name="country_of_origin" label="Країна походження" type="text" />
        <AppTextArea />
  ...
  <div class="flex justify-center">
    <button class="px-6 h-10 flex justify-center items-center rounded-full bg-black text-white hover:shadow-sm"
      @click="sendData" ref="el" @mouseenter="startAnimation" @mouseleave="stopAnimation">
      Відправити
    </button>
  </div>
</template>
...
import AppTextArea from '@/components/atoms/inputs/AppTextArea.vue';
